...
getOnlineCustomer(_businessController.selectedBusiness.value!.businessId!);
clearValue();
...
fileId= await _uploadFileController.uploadFile(CustomerImage.value!.path);
   
      }
var response= await http.put(Uri.parse(ApiLink.add_customer+"/"+customer.customerId!),body:jsonEncode(
  {
...
var result=await _businessController.sqliteDb.getOfflineCustomers(businessId);
...
final response=await http.get(Uri.parse(ApiLink.get_business_customer+"?businessId="+businessId),headers: {
...
var result=List.from(json['data']['content']).map((e) => Customer.fromJson(e)).toList();
...
var response=await http.delete(Uri.parse(ApiLink.add_customer+"/${customer.customerId}?businessId=${customer.businessId}"),headers: {
